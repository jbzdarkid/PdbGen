...
real_out2 = subprocess.run([exe, 'dump', '--all', real], capture_output=True)
if real_out.stderr:
    print('Real: ', real_out.stderr)
    exit()
fake_out = subprocess.run([exe, 'pdb2yaml', '--all', fake], capture_output=True)
fake_out2 = subprocess.run([exe, 'dump', '--all', fake], capture_output=True)
if fake_out.stderr:
